...
public class DotNetLinker : IDotNetLinker
{
    ...
    /// </summary>
    public void LinkNamespace(string namespaceName)
    {
        LoggerService.Logger.Info($"Linking namespace: {namespaceName}");

        _linkedNamespaces.Add(namespaceName);

        // Try to load the corresponding assembly
        string assemblyName = GetAssemblyNameFromNamespace(namespaceName);
