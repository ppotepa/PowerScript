    ...
    public class DiagnosticAnalyzer
    {
        ...
        private void CollectLinkedLibraries(Token[] tokens)
        {
            for (int i = 0; i < tokens.Length; i++)
            {
                if (tokens[i] is LinkKeywordToken && i + 1 < tokens.Length)
                {
                    ...
                    {
                        var libraryName = identifierToken.RawToken.Text.ToUpperInvariant();
        ...
        private void AnalyzeLibraryUsage(Token[] tokens)
        {
            for (int i = 0; i < tokens.Length - 2; i++)
            {
                ...
                {
                    var libraryName = libToken.RawToken.Text.ToUpperInvariant();
                    var usage = $"{libToken.RawToken.Text}::{tokens[i + 2].RawToken.Text}";
                ...
                if (tokens[i] is IdentifierToken nameToken &&
                    ...
                    tokens[i + 2] is IdentifierToken)
                {
                    var fullPath = $"{nameToken.RawToken.Text}.{tokens[i + 2].RawToken.Text}";
                    var rootNamespace = nameToken.RawToken.Text.ToUpperInvariant();
        ...
        {
            var functionName = declaration.Identifier.RawToken.Text;
        ...
        private void CheckUnusedLibraries()
        {
            ...
            foreach (var library in _linkedLibraries)
            {
                if (library == "SYSTEM" || library == "MATH" || library == "IO")
                {
        ...
        /// </summary>
        private bool IsWellKnownNamespace(string namespaceName)
        {
            var wellKnownNamespaces = new[]
            {
