...
internal class IfStatementProcessor : ITokenProcessor
{
    ...
    public TokenProcessingResult Process(Token token, ProcessingContext context)
    {
        ...
        Console.WriteLine($"[IfStatementProcessor] Processing IF statement in scope '{context.CurrentScope.ScopeName}'");
        Console.ResetColor();
        ...
        var thenScope = new Scope(thenScopeName)
        {
        ...
        var ifStatement = new IfStatement(condition, thenScope, elseScope);
    ...
    private Expression ParseComparisonExpression(ref Token currentToken)
    {
        ...
        {
            throw new Exception($"Expected comparison operator (>, <, >=, <=, ==, !=), got {currentToken?.GetType().Name}");
        }
