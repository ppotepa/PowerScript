...
public class Scope
{
    ...
    public void Visualize(int depth = 0)
    {
        ...
        if (Decarations.Count > 0)
        {
            ...
            foreach (KeyValuePair<string, Declaration> decl in Decarations)
            {
    ...
    {
        if (_variableTypes.TryGetValue(name, out Type? type))
        {
            return type;
        }
    ...
    public bool ValidateTypeAssignment(string variableName, Type valueType)
    {
        ...
        Type? declaredType = GetVariableType(variableName);
