    ...
    public class FunctionProcessor(ParameterProcessor parameterProcessor) : ITokenProcessor
    {
        ...
        public TokenProcessingResult Process(Token token, ProcessingContext context)
        {
            ...
            if (isAlternativeSyntax)
            {
                ...
                LoggerService.Logger.Debug($"Delegating parameter processing for function '{functionName}' (alternative syntax)");

                // Parse parameters until we hit the opening brace
                var (altParametersToken, altNextToken) = ParseParametersAlternativeSyntax(currentToken.Next);
        ...
        /// </summary>
        private (FunctionParametersToken, Token) ParseParametersAlternativeSyntax(Token startToken)
        {
            ...
            while (currentToken is not ScopeStartToken)
            {
                ...
                var implicitTypeToken = new IntToken();
