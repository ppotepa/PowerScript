...
namespace Tokenez.Compiler
{
    ...
    {
        private readonly Tokenez.Parser.Lexer.TokenTree _tree;
        private readonly Dictionary<string, object> _variables = [];
        private int _recursionDepth = 0;
        private const int MaxRecursionDepth = 1000;
        private readonly List<string> _callStack = [];

        public PowerScriptCompiler(Tokenez.Parser.Lexer.TokenTree tree)
        {
        ...
        private List<object> ParseFunctionArguments(Token? startToken)
        {
            ...
            while (current != null && !(current is ParenthesisClosed && parenDepth == 0))
            {
                ...
                else if (current is CommaToken && parenDepth == 0)
                {
                    ...
                    {
                        var argExpression = BuildExpressionFromTokens(argumentTokens);
                        var argValue = EvaluateExpressionValue(argExpression);
        ...
        private Expression BuildExpressionFromTokens(List<Token> tokens)
        {
            ...
            if (tokens.Count == 1)
            {
                ...
                else if (token is StringLiteralToken strToken)
                {
                    return new StringLiteralExpression(strToken);
                }
            ...
            if (operatorIndex >= 0)
            {
                ...
                Expression left = BuildExpressionFromTokens(leftTokens);
                Expression right = BuildExpressionFromTokens(rightTokens);
                
                return new BinaryExpression(left, tokens[operatorIndex], right);
            }

            // No operator found - maybe parenthesized expression?
            if (tokens[0] is ParenthesisOpen && tokens[^1] is ParenthesisClosed)
            {
                return BuildExpressionFromTokens(tokens.Skip(1).Take(tokens.Count - 2).ToList());
            }

            throw new InvalidOperationException($"Cannot build expression from tokens: {string.Join(", ", tokens.Select(t => t.GetType().Name))}");
        }
