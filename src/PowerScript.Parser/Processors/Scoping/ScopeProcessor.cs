...
namespace PowerScript.Parser.Processors.Scoping;
...
/// </summary>
public class ScopeProcessor(ITokenProcessorRegistry registry, IScopeBuilder scopeBuilder) : ITokenProcessor
{
    ...
    {
        ScopeStartToken? scopeStartToken = token as ScopeStartToken;
        ...
        Token? prevToken = scopeStartToken!.Prev;
        while (prevToken is not null and not FunctionToken)
        {
        ...
        if (prevToken is FunctionToken)
        {
            ...
            if (prevToken.Next is IdentifierToken functionNameToken)
            {
                ...
                {
                    FunctionDeclaration? declaration = context.CurrentScope.Decarations[functionName] as FunctionDeclaration;
        ...
        while (currentToken is not null and not ScopeEndToken)
        {
            ...
            if (processor != null)
            {
                TokenProcessingResult result = processor.Process(currentToken, scopeContext);
                ...
                if (result.ModifiedScope != null)
                {
                    ...
                    while (nestedToken != null && braceCount > 0)
                    {
                        ...
                        if (nestedToken is ScopeStartToken)
                        {
                            braceCount++;
                        }

                        if (nestedToken is ScopeEndToken)
                        {
                            braceCount--;
                        }
                    }
